function Zhuanpan(opt){
	...
	this.trynum = 5;//尝试5次，还是不行的话就提示网络异常,请稍后重试
	...
	this.global_ajaxdomain = 'http://www.lenovouat.cn/srv/';
...
Zhuanpan.prototype = {
	...
	bind : function(){
		...
		this.$startBtn.click(function(){
			...
			if(!passport.isLogin()){//登陆成功
				/*console.log('lenovo failure');
				islogin_flag = false;
				var passport_login = passport.login || function(){};
				passport.show();
	...
	scan : function(){//检查是否已返回值
		//console.log(this.ajax_return_flag+'------00---'+this.chance_num);
		this.trynum--;
		if(this.trynum <= 0){
	...
	stop : function(){
		if(this.prizetype==1){//非实物中奖
			this.opt.gongxizhongjiang_show && this.opt.gongxizhongjiang_show(this.msg);
		}else{//实物中奖
			this.opt.wanshandizhi_show && this.opt.wanshandizhi_show(this.msg);
		}
	...
	validate : function(){
		...
		if(!this.islogin_flag){
			this.opt.nologin_show && this.opt.nologin_show();
			return 0;
		}
	...
	ajax : {
		...
		choujiang : function(_this,opt){
			opt.url = _this.global_ajaxdomain + 'lotterydrawtest.do?plat=4&rotatingwheelid=740b7523-35c6-4d1d-8191-1c9863e0e1eb';
			opt.dataType = 'jsonp';
			this.common(opt);
		},
		choujiangcishu : function(_this,opt){
			opt.url = _this.global_ajaxdomain + 'canlotterydraw.do?plat=4&rotatingwheelid=740b7523-35c6-4d1d-8191-1c9863e0e1eb';
			opt.dataType = 'json';
		...
		heros : function(_this,opt){
			opt.url = _this.global_ajaxdomain + 'lotterydrawhistory.do?plat=4&rotatingwheelid=740b7523-35c6-4d1d-8191-1c9863e0e1eb';
			opt.dataType = 'json';
			opt.sucFn = function(msg){
				if(msg.rc==0){
